...
if($event!=""){
$tag_data=InfoTag::tagDataId($id);

if($tag_data["level"]==1){
InfoTag::Event1($event,INFO_TAG_TABLE,$id);
}
InfoTag::Event2($event,INFO_TAG_TABLE,$id);
}
	
$tag1_list=InfoTag::tagListShowLevel($level=1);
$tag2_list=InfoTag::tagListShowLevel($level=2);
...
$table=INFO_TAG_TABLE;
$table2=INFO_TAG_TABLE_LANG;
if($id!=""&&$id!="new"){
	$where['id']=$id;
	$db = new mysql;
...
$info_navigate=$tpl->get('menu-navigate');

return $info_navigate;
}
...
/*
if($event!=""){FotoInfo::Event($event,'items_branch',$id);}
if($com=="add"){
